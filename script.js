...
function initGame() {
    ...
    const gameBoard = document.getElementById('game-board'); // initialize game board variable
    gameBoard.innerHTML = ''; // set HTML to empty
    
    cards = [...symbols, ...symbols]; // put symbols into cards array twice (pairs)
    shuffleArray(cards); // shuffle the symbols

    cards.forEach(symbol => { // for each symbol in cards..
        card = createCard(symbol); // create a card with that symbol
        gameBoard.appendChild(card); // put card into game board
    });
...
function createCard(symbol) {
    // Write your code here
    const card = document.createElement('div'); // initialize card
    card.classList.add('card'); // give it class name 'card'
    card.dataset.symbol = symbol; // give card a symbol
    card.addEventListener('click' , () => flipCard(card)); // when clicking the card, flip it
...
function flipCard(card) {
    ...
    // Write your code here
    card.classList.add('flipped'); // add flipped class to flipped card
    card.textContent = card.dataset.symbol; // when card is flipped, display its symbol

    if (!firstCard) { // if this is first card picked..
        firstCard = card; // set the card = to firstCard
    } else {
        secondCard = card; // else set the card = to secondCard
        checkForMatch(); // and check for match
    }
}
...
function checkForMatch() {
    ...
    if (firstCard.dataset.symbol === secondCard.dataset.symbol) {
        disableCards(); // set the cards to matched
    } else {
...
function disableCards() {
    // Write your code here
    firstCard.classList.add('matched'); // set firstCard to match secondCard
    secondCard.classList.add('matched'); // set secondCard to match firstCard
